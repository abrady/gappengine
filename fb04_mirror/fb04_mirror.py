...
class BaseHandler(webapp.RequestHandler):
    ...
    def graph(self):
        ...
        if not hasattr(self._graph, "access_token") and self.current_user:
            self._graph.access_token = self.current_user.access_token
        return self._graph
...
class HomeHandler(BaseHandler):
    def get(self):
        ...
        if graph_path:
            post_args = self.request.get("post_args")
            if graph_path[0:2] == 'me':
